...
public class SenceController : MonoBehaviour
{
    ...
    private void MoveModel()
    {
        ...
        Vector3 offset = new Vector3(0, 0, 0); //鼠标点击位置与模型之间的偏移量
        ...
        if (isMoveModel && target)
        {
            ...
            //if(mouseSpeed*2>20)
            //    target.transform.position = Vector3.MoveTowards(target.transform.position, worldPos, 20 * Time.deltaTime);
            //else
            //    target.transform.position = Vector3.MoveTowards(target.transform.position, worldPos, mouseSpeed*2 * Time.deltaTime);
